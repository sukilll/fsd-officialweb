...
const BlogPost = () => {
  ...
  const renderContent = (content: string) => {
    return content
      ...
      .map((line, index) => {
        if (line.startsWith('# ')) {
          return <h1 key={index} className="text-3xl font-bold mt-12 mb-6 text-black">{line.slice(2)}</h1>;
        }
        if (line.startsWith('## ')) {
          return <h2 key={index} className="text-2xl font-semibold mt-10 mb-4 text-black">{line.slice(3)}</h2>;
        }
        if (line.startsWith('### ')) {
          return <h3 key={index} className="text-xl font-medium mt-8 mb-3 text-black">{line.slice(4)}</h3>;
        }
        if (line.startsWith('- ')) {
        ...
        return <p key={index} className="mb-6 text-gray-700 leading-relaxed text-lg">{line}</p>;
      });
  };
